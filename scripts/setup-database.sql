-- Simple Database Setup for Credit Report System
...
-- Create the credit_reports table if it doesn't exist
...
-- Create credit_negative_items table if it doesn't exist
CREATE TABLE IF NOT EXISTS credit_negative_items (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    credit_report_id UUID NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_credit_accounts_creditor ON credit_accounts(creditor_name);

CREATE INDEX IF NOT EXISTS idx_negative_items_report_id ON credit_negative_items(credit_report_id);
CREATE INDEX IF NOT EXISTS idx_negative_items_type ON credit_negative_items(item_type);

CREATE INDEX IF NOT EXISTS idx_inquiries_report_id ON credit_inquiries(credit_report_id);
CREATE INDEX IF NOT EXISTS idx_inquiries_date ON credit_inquiries(inquiry_date);
