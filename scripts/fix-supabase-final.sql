-- Final Fix for Supabase Functions
-- This script removes dependency on auth.users table and uses only our custom tables

-- The issue is that the functions are trying to access auth.users table
-- which requires special permissions. We'll modify the functions to work
-- with only our custom mlm_users table.

-- Fix 1: Drop and recreate get_channel_messages function
...
-- Fix 2: Drop and recreate get_user_channels function
    ...
    SELECT 
        ...
        COALESCE(
            (SELECT jsonb_build_object(
                ...
                'sender_name', COALESCE(mu.display_name, mu.email, 'Test User')
            )
