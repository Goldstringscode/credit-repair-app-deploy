...
interface Channel {
  ...
  channel_type: string; unread_count: number; member_count: number
}
interface Message {
  id: string; channel_id: string; sender_id: string; sender_name: string
  content: string; message_type: string; attachment_url?: string
  attachment_name?: string; created_at: string
}
...
export default function CommunicationsPage() {
  ...
  // Auth check on mount - uses httpOnly cookie automatically
  useEffect(()=>{
  ...
  const selectChannel = useCallback(async(ch:Channel)=>{
    unsubRef.current?.()
    setActive(ch); setMessages([]); setLoading(true); setError(null)
    try {
      const res = await fetch('/api/mlm/communications/messages?channelId='+ch.id+'&limit=50')
      const data = await res.json()
    ...
    const channel = supabase.channel('mlm-ch-'+ch.id)
      ...
      .subscribe(s=>{
        if(s==='SUBSCRIBED') setConnStatus('connected')
        else if(s==='CLOSED') setConnStatus('disconnected')
        else if(s==='CHANNEL_ERROR') setConnStatus('error')
  ...
  const handleFile = async(e:React.ChangeEvent<HTMLInputElement>)=>{
    ...
    try {
      ...
      const {error:upErr}=await supabase.storage.from('mlm-files').upload(path,file,{upsert:false})
      if(upErr) throw new Error(upErr.message)
      setUploadPct(80)
      const {data:urlData}=supabase.storage.from('mlm-files').getPublicUrl(path)
  ...
  const sc:Record<ConnStatus,string>={connected:'#22c55e',connecting:'#f59e0b',disconnected:'#6b7280',error:'#ef4444'}
      ...
      <aside style={{width:260,borderRight:'1px solid var(--color-border-tertiary)',display:'flex',flexDirection:'column',overflowY:'auto',background:'var(--color-background-primary)'}}>
        ...
        {channels.map(ch=>(
          <button key={ch.id} onClick={()=>selectChannel(ch)}
            ...
            <span style={{fontSize:14,color:active?.id===ch.id?'var(--color-text-primary)':'var(--color-text-secondary)',fontWeight:active?.id===ch.id?500:400}}>
              # {ch.name}
            </span>
            {(ch.unread_count||0)>0&&(
              <span style={{background:'#3b82f6',color:'white',borderRadius:10,fontSize:11,fontWeight:600,padding:'1px 6px',minWidth:18,textAlign:'center'}}>
                {ch.unread_count}
              </span>
            )}
          </button>
      ...
      <main style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
        ...
        <div style={{padding:'12px 20px',borderBottom:'1px solid var(--color-border-tertiary)',display:'flex',alignItems:'center',justifyContent:'space-between',background:'var(--color-background-primary)'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <span style={{fontSize:12,color:'var(--color-text-secondary)'}}>{connStatus}</span>
        ...
        {/* Error */}
        {error&&<div style={{margin:'0 20px 8px',padding:'8px 14px',background:'#fef2f2',color:'#dc2626',borderRadius:6,fontSize:13,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
          {error}<button onClick={()=>setError(null)} style={{background:'none',border:'none',cursor:'pointer',color:'inherit',fontSize:16}}>✕</button>
        </div>}
        ...
        <div style={{padding:'12px 20px',borderTop:'1px solid var(--color-border-tertiary)',display:'flex',gap:8,background:'var(--color-background-primary)'}}>
          ...
          <button onClick={()=>fileInputRef.current?.click()} disabled={!active}
            style={{padding:'8px 12px',borderRadius:6,border:'1px solid var(--color-border-secondary)',background:'transparent',cursor:active?'pointer':'not-allowed',fontSize:16,color:'var(--color-text-secondary)',flexShrink:0}}
          ...
          <button onClick={handleSend} disabled={!input.trim()||!active}
            style={{padding:'8px 18px',borderRadius:6,border:'none',background:input.trim()&&active?'#3b82f6':'var(--color-border-secondary)',color:input.trim()&&active?'white':'var(--color-text-tertiary)',cursor:input.trim()&&active?'pointer':'not-allowed',fontSize:14,fontWeight:500,flexShrink:0,transition:'background 0.15s'}}>
            Send
          </button>
        </div>
      </main>
