'use client'
/**
 * app/mlm/communication/page.tsx
 * Uses Supabase Realtime instead of custom server.js WebSocket.
 * Works on Vercel serverless with zero extra infrastructure.
 */
...
export default function MLMCommunicationPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setCurrentUser({ id: session.user.id, name: session.user.email ?? 'Member' })
      try {
        const userChannels = await fetchUserChannels(session.access_token)
        setChannels(userChannels)
        if (userChannels.length > 0) selectChannel(userChannels[0], session.access_token, session.user.id, session.user.email ?? 'Member')
      } catch { setError('Failed to load channels. Please refresh.') }
    }
    init()
  ...
  const selectChannel = useCallback(async (channel: MLMChannel, token: string, userId: string, userName: string) => {
    unsubRef.current?.(); presenceRef.current?.()
    setActiveChannel(channel); setMessages([]); setIsLoadingMessages(true); setError(null)
    try { setMessages(await fetchChannelMessages(channel.id, token)) } catch { setError('Failed to load messages.') } finally { setIsLoadingMessages(false) }
  ...
  return (
    <div style={{ display: 'flex', height: 'calc(100vh - 64px)', fontFamily: 'var(--font-sans)' }}>
      <aside style={{ width: 260, borderRight: '1px solid var(--color-border-tertiary)', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
        ...
        {channels.map(ch => (
          <button key={ch.id} onClick={() => authToken && currentUser && selectChannel(ch, authToken, currentUser.id, currentUser.name)}
      ...
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--color-border-tertiary)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: statusColor[connectionStatus], display: 'inline-block' }} />
          </div>
        </div>
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
          {isLoadingMessages && <div style={{ textAlign: 'center', color: 'var(--color-text-tertiary)', padding: 40 }}>Loading...</div>}
          {!isLoadingMessages && messages.length === 0 && activeChannel && <div style={{ textAlign: 'center', color: 'var(--color-text-tertiary)', padding: 40 }}>No messages yet. Be the first!</div>}
          {messages.map(msg => (
            <div key={msg.id} style={{ marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
                <span style={{ fontWeight: 500, fontSize: 14 }}>{msg.sender_name}</span>
