...
export async function POST(req: NextRequest) {
  ...
  // Personal volume — from DB only, never client-supplied
  ...
  const { data: tvData } = await supabase.rpc('get_mlm_team_volume', {
    p_mlm_user_id: mlmUser.id,
    p_since: monthStart,
  })
  const teamVolume = (tvData as number) ?? 0

  // Active direct downlines — subquery done server-side via RPC to avoid string injection
  const { data: downlineData } = await supabase.rpc('get_mlm_active_downline_count', {
    p_sponsor_mlm_id: mlmUser.id,
  })
  const activeDownlines = (downlineData as number) ?? 0

  const earnedRank = computeEarnedRank(personalVolume, teamVolume, activeDownlines)
  ...
  if (earned.level > current.level) {
    ...
    if (bonus > 0) {
      await supabase.from('mlm_commissions').insert({
        ...
        recipient_mlm_id: mlmUser.id,
        source_user_id: userId,
        commission_type: 'rank_advancement',
        commission_amount: bonus,
        status: 'payable',
        payable_at: now.toISOString(),
        created_at: now.toISOString(),
      })
    }

    await supabase.from('mlm_audit_log').insert({
      event_type: 'rank_advancement',
      user_id: userId,
      details: {
  ...
  return NextResponse.json({
    ...
    currentRank,
    earnedRank,
    metrics: { personalVolume, teamVolume, activeDownlines },
  })
}