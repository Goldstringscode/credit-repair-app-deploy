...
// Derived stats and helpers (previously referenced but never defined —
// this was a separate pre-existing bug on top of the missing closing brace)
...
l.letterType.toLowerCase().includes(searchTerm.toLowerCase()) ||
l.trackingNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
l.recipient.toLowerCase().includes(searchTerm.toLowerCase())
const matchesStatus = statusFilter === 'all' || l.status === statusFilter
const matchesBureau = bureauFilter === 'all' || l.creditBureau === bureauFilter
return matchesSearch && matchesStatus && matchesBureau
})

const getStatusColor = (status: LetterTracking['status']) => ({
...
const getStatusIcon = (status: LetterTracking['status']) => {
if (status === 'delivered') return <CheckCircle className="h-3 w-3" />
if (status === 'in_transit') return <Truck className="h-3 w-3" />
if (status === 'returned') return <AlertCircle className="h-3 w-3" />
